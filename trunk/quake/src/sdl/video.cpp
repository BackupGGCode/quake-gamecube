...
namespace quake
{
	namespace video
	{
		...
		// Render buffers.
		static std::vector<pixel_t>			render_buffer;
...
void VID_Init(unsigned char* palette)
{
	...
	depth_buffer.resize(screen->w * screen->h);
	surface_cache.resize(D_SurfaceCacheForRes(screen->w, screen->h));

	// Create the back buffer.
	back_buffer = SDL_CreateRGBSurfaceFrom(
		&render_buffer.at(0),
		screen->w,
		screen->h,
		8,
...
{
	// Copy from render buffer to screen.
	for (const vrect_t* rect = rects; rect != 0; rect = rect->pnext)
	{
		SDL_Rect src_rect	= {rect->x, rect->y, rect->width, rect->height};
		SDL_Rect dst_rect	= src_rect;
		SDL_BlitSurface(back_buffer, &src_rect, screen, &dst_rect);
	}
	SDL_Flip(screen);
}
